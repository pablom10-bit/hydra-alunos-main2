...
const { getActiveLaunchUsers } = require("./repositories/usersRepository");
...
async function sendLaunchNotifications(launches, WSS, broadcastLabel) {
    const settings = await getDefaultSettings();
    const users = await getActiveLaunchUsers();

    const subject = "New launches at Binance! - Hydra Warning";

    const emailMessage = `Hi, [USER]!
    
    Hydra Launch Monitor found new launch(es) at Binance:
    
    <a href="https://launchpad.binance.com/pt-BR/" title="Click to seet at Binance">
        ${launches.map(launch => `${launch.token} (${launch.type})<br />`)}
    </a>
    ...
    const telegramMessage = `Hi, [USER]!
    
    Hydra Launch Monitor found new launch(es) at Binance:
    
    ${launches.map(launch => `${launch.token} (${launch.type})\n\t`)}

    See at: https://launchpad.binance.com/pt-BR/
    ...
    for (let i = 0; i < users.length; i++) {
        ...
        WSS.direct(user.id, {
            notification: {
                type: "success",
                text: `New Launches: ${JSON.stringify(launches.map(l => l.token))}`
            }
        });

        await sendEmail(settings, emailMessage.replace("[USER]", user.name), user.email, subject);
        if (user.telegramChat)
            await sendTelegram(settings, telegramMessage.replace("[USER]", user.name), user.telegramChat);
    }
}

async function startLaunchMonitor(monitorId, broadcastLabel, logs) {
    const launchesRepository = require("./repositories/launchesRepository");

    const checkLaunches = async () => {
        const launchScrap = await Exchange.checkLaunches();

        const newLaunches = [];

        const launchpoolExists = await launchesRepository.launchExists(launchScrap.launchpool);
        if (!launchpoolExists) {
            newLaunches.push({ token: launchScrap.launchpool, type: "LAUNCHPOOL" });
            await launchesRepository.insertLaunch(newLaunches[0]);
        }

        const launchpadExists = await launchesRepository.launchExists(launchScrap.launchpad);
        if (!launchpadExists) {
            newLaunches.push({ token: launchScrap.launchpad, type: "LAUNCHPAD" });
            await launchesRepository.insertLaunch(newLaunches[newLaunches.length - 1]);
        }

        if (newLaunches.length > 0) {
            await sendLaunchNotifications(newLaunches, WSS, broadcastLabel);
            if (logs) logger("M:" + monitorId, `New Launches found: ${JSON.stringify(launchScrap)}`);
        }
        else if (logs) logger("M:" + monitorId, `There aren't new launches by now.`);
    }

    checkLaunches();
    setInterval(checkLaunches, 60 * 60 * 1000);

    logger("M:" + monitorId, "Launch Monitor has started!");
}
...
async function init(users, wssInstance, hydraInstance) {
    ...
    const launchMonitor = monitors.find(m => m.type === monitorTypes.LAUNCHES);
    if (launchMonitor) startLaunchMonitor(launchMonitor.id, launchMonitor.broadcastLabel, launchMonitor.logs);
